...
class Database:
    ...
    def __init__(self,
        ...
        # ----------------------------------- SQL QUERIES FOR IN-DB (UN)FIT -------------------------------------------------------------
        

        self._partial_fit_sql = """ 
            ...
            counts as (
                SELECT item, feature, weight as w, SUM(weight) OVER(PARTITION BY item)::float AS norm
                FROM featurize('{train_table}', {attributes}, {fkeys})
            )
            SELECT feature, class, SUM(w/norm) as weight
            FROM counts JOIN targets USING (item)
    ...
    #-----------------------------------------------------------------------------------------------------------
    # NEW CODE: 
    def update_corpus(self, con, train_table, attributes, foreign_keys):
        ...
        """ Implements partial fit, but instead of writing items into a table, fits the model from normalized db
            ...
            Args:
                ...
                attributes::List[str] = a list of qualified attribute/column names
        ...
        if not self.exists(con, self.table_corpus):
            self.table_corpus.create(con)

        return con.execute(
            text(self._partial_fit_sql.format(
    ...
    # MODIFIED partial_fit
    def partial_fit(self, con, X, y=None, attributes=None, foreign_keys=None, sample_weight=None):
        if attributes is not None:
            self.update_corpus(con, train_table=X, attributes=attributes, foreign_keys=foreign_keys)
        else:
    ...
    # MODIFIED predict
    def predict(self, con, X, attributes=None, fkeys=None):
        cache = self.is_deployed(con)
        view = ""
        
        if isinstance(X, str) & (attributes is not None):
            print("Collecting items_table from db.")
            items = "items_table" # tabella con gli indici da classificare
            attributes = "'{" +  ", ".join('"'+col+'"' for col in attributes) + "}'"
            fkeys = "'{" +  ", ".join('"'+fk+'"' for fk in fkeys) + "}'"

            view = f""" 
                    CREATE OR REPLACE VIEW items_table AS
                    SELECT item, feature, weight
                    FROM featurize('{X}', {attributes}, {fkeys});
                """
        else:
            print("Writing items into db.")
            items = self.write_items(con, X)
        sql = view + self._sql_predict(cache, items)
    ...
    def _sql_predict(self, cache, items):
        # preprend the featurization step: create `items_table` view
        return f"""
