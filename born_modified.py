...
class BornClassifierSQL:
    ...
    def fit_sql(self, train_table, attributes=None, fkeys=None, sample_weight=None):
        with self.db.connect() as con:
            with con.begin():
                self.db.check_editable(con)
                self.db.table_corpus.drop(con, checkfirst=True)
                self.db.partial_fit(con, X=train_table, attributes=attributes, foreign_keys=fkeys, sample_weight=sample_weight)

        return self
    
    def predict_sql(self, train_table, attributes=None, fkeys=None):
        with self.db.connect() as con:
            with con.begin():
                self.db.check_fitted(con)
                classes = self.db.predict(con, X=train_table, attributes=attributes, fkeys=fkeys)
        return classes
    ...
    def predict(self, X):
        ...
        with self.db.connect() as con:
            ...
            items = classes[self.db.n]
            
        #classes = dict(zip(classes[self.db.n], classes[self.db.k]))
        #classes = [classes[i] if i in classes else None for i in items]
